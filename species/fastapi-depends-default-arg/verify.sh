#!/bin/sh
# fastapi-depends-default-arg verifier (Sprint 024 Python/FastAPI wave, command:
...
# this proves every Python file STILL PARSES after the mutable default was
# rewritten — the parse gate the sprint-024 contract requires in place of the
# (vacuous on a non-Go repo) Go-build compile check. A rewrite that left a syntax
# error makes `py_compile` exit non-zero, the check fails, and the fix is skipped
# (never staged).
#
