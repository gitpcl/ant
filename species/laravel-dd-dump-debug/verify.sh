#!/bin/sh
# laravel-dd-dump-debug verifier (Sprint 023 PHP/Laravel wave, command: escape
...
# this proves every PHP file STILL PARSES after the debug statement was removed —
# the parse gate the sprint-023 contract requires in place of the (vacuous on a
# non-Go repo) Go-build compile check. A delete that left a syntax error makes
# `php -l` exit non-zero, the check fails, and the fix is skipped (never staged).
#
