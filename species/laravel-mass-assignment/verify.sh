#!/bin/sh
# laravel-mass-assignment verifier (Sprint 023 PHP/Laravel wave, command: escape
...
# this proves every PHP file STILL PARSES after the mass-assignment argument was
# narrowed to validated/whitelisted input — the parse gate the sprint-023 contract
# requires in place of the (vacuous on a non-Go repo) Go-build compile check. A
# rewrite that left a syntax error makes `php -l` exit non-zero, the check fails,
# and the fix is skipped (never staged).
#
