#!/bin/sh
# python-debug-print verifier (Sprint 024 Python/FastAPI wave, command: escape
...
# this proves every Python file STILL PARSES after the debug statement was removed
# — the parse gate the sprint-024 contract requires in place of the (vacuous on a
# non-Go repo) Go-build compile check. A removal that left a syntax error makes
...
# non-zero only on a real syntax error). RequiredTools=["python3"] in the fixture
# skips this case when python3 is absent, exactly as the ast-grep species skip
# without the matcher — so CI without python3 stays green while the gate runs for
# real where present.
set -eu
