#!/bin/sh
# python-sql-fstring verifier (Sprint 024 Python/FastAPI wave, command: escape
...
# this proves every Python file STILL PARSES after the f-string SQL was rewritten
# to a parameterized query — the parse gate the sprint-024 contract requires in
# place of the (vacuous on a non-Go repo) Go-build compile check. A rewrite that
# left a syntax error makes `py_compile` exit non-zero, the check fails, and the
# fix is skipped (never staged).
#
...
# or run module top-level code). Authoring a SECURITY species that ran untrusted,
# post-diff Python at verify time would itself be a code-execution surface; the
# parse gate is deliberately execution-free.
set -eu
